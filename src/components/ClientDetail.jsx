...
export default function ClientDetail({ client, onClose }) {
  ...
  const handleCancel = async () => {
    if (window.confirm(`Cancelar o cliente "${client.name}"?`)) {
  ...
  const handleDelete = async () => {
    if (window.confirm(`EXCLUIR PERMANENTEMENTE o cliente "${client.name}"?`)) {
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
            {PRIORITY_LEVELS.map(level => {
              ...
              return (
                <button key={level} onClick={() => setClientPriority(client.id, level)} style={{
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            {[
              ...
              <a key={l.icon} href={l.url} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', background: '#1a1a1a', borderRadius: 8, marginBottom: 6, border: '0.5px solid var(--border)', color: 'var(--text-muted)', fontSize: 13 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {clientReports.slice(0, 2).map(r => (
                  <div key={r.id} style={{ background: '#1a1a1a', borderRadius: 8, padding: '8px 12px', border: '0.5px solid var(--border)' }}>
                    <div style={{ fontSize: 11, color: '#555', marginBottom: 3 }}>{new Date(r.date + 'T00:00:00').toLocaleDateString('pt-BR')} às {r.time}</div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            {demands.length === 0 && !showNewDemand && <div style={{ fontSize: 13, color: '#333', padding: '8px 0' }}>Nenhuma demanda</div>}
            {demands.map(d => (
              <div key={d.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', background: '#1a1a1a', borderRadius: 8, marginBottom: 6, border: '0.5px solid var(--border)' }}>
                <div onClick={() => toggleDemand(d.id)} style={{ width: 16, height: 16, borderRadius: 4, flexShrink: 0, cursor: 'pointer', border: `0.5px solid ${d.done ? '#639922' : '#333'}`, background: d.done ? '#1a2e12' : '#141414', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          {/* Observações */}
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 11, color: 'var(--orange)', letterSpacing: 0.5, textTransform: 'uppercase', marginBottom: 8, paddingBottom: 6, borderBottom: '0.5px solid #1f1f1f' }}>Observações</div>
            <textarea rows={3} placeholder="Escreva anotações sobre este cliente..." value={notes} onChange={e => { setNotes(e.target.value); setNotesSaved(false) }} onBlur={saveNotes} />
